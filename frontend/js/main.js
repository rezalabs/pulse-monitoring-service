...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function updateLogoutButtonVisibility () {
    ...
    logoutBtn.classList.toggle('hidden', !secret)
  }

  /**
     * Retrieves the admin secret, first checking sessionStorage, then prompting the user.
     * @param {string} title - The title for the secret modal.
     * @returns {Promise<string|null>} A promise that resolves with the secret, or null.
     */
  function getAdminSecret (title) {
    ...
    forgetAdminSecret() // Clear any expired secret
  ...
  function renderChecks (checks) {
    ...
    if (checks.length === 0 && currentPage === 1) {
      checkListContainer.innerHTML = '<p class="no-checks-message">No checks have been configured. Click "Add Check" to get started.</p>'
      return
    }
    ...
    checkListContainer.innerHTML = checks.map(check => {
      ...
      return `
                <div class="check-item" data-uuid="${check.uuid}">
                    <div class="check-status">
                        <span class="status-indicator status-${check.status}" title="Status: ${check.status}">${check.status}</span>
                    </div>
                    <div class="check-info">
                        <div class="name">${check.name}</div>
                        <div class="details" title="${lastPingAbsolute}">
                            Last ping: ${lastPingRelative} &bull; Duration: ${duration}
                        </div>
                        <div class="ping-url">
  ...
  checkForm.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      hideCheckModal()
      loadAndRenderChecks(currentPage === 1 ? 1 : Math.ceil((meta.total + 1) / limit)) // Go to last page
    } catch (error) {
  ...
  (function initialize () {
    ...
    setInterval(() => loadAndRenderChecks(currentPage), 30000) // Refresh every 30 seconds
  })()
})
